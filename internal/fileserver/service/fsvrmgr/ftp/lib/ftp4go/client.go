...
func (ftp *FTP) UploadResumeFile(remotename string, localpath string, useLineMode bool, callback Callback) (err error) {
	var f *os.File
	f, err = os.Open(localpath)
	defer f.Close()
	...
	if useLineMode {
		...
		var offset int = 0
		//?int not enough
		offset, err = ftp.Size(remotename)
		if err != nil{
			return
		}

		if err = ftp.UresumeFile(APPE_FTP_CMD, f, int64(offset), BLOCK_SIZE, remotename, localpath, callback); err != nil {
			return err
		}
	}
...
func (ftp *FTP) UresumeFile(cmd FtpCmd, reader *os.File, offset int64, blocksize int, remotename string, filename string, callback Callback) (err error) {
	var conn net.Conn
	if _, err = ftp.SendAndRead(TYPE_I_FTP_CMD); err != nil {
		return
	}
	...
	separateCall := func() error {
		if offset != 0{
			//move file pointer to the upload point
			reader.Seek(offset, io.SeekStart)
			//if res, err := ftp.SendAndRead(APPE_FTP_CMD, fmt.Sprintf("%s", remotename)); err != nil{
			//	return err
			//}else{
			//	fmt.Println(res)
			//}
		...
		ftp.writeInfo("Try and restore bytes via connection for remote address:", conn.RemoteAddr().String())
